...
/* From 21st.dev — Stepper component */
...
function StepperTrigger({ asChild = false, className, children, tabIndex, ...props }: StepperTriggerProps) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLButtonElement>) => {
    switch (e.key) {
      ...
      case "Home":
        e.preventDefault(); if (focusFirst) focusFirst(); break
      case "End":
        e.preventDefault(); if (focusLast) focusLast(); break
      case "Enter": case " ":
        e.preventDefault(); setActiveStep(step); break
    }
  }
  ...
  return (
    <button
      ...
      data-state={state}
      className={cn(
        "cursor-pointer inline-flex items-center gap-3 rounded-full outline-none focus-visible:z-10 focus-visible:ring-2 focus-visible:ring-primary disabled:pointer-events-none disabled:opacity-60",
        className
      )}
...
function StepperIndicator({ children, className }: React.ComponentProps<"div">) {
  ...
  return (
    <div
      ...
      data-state={state}
      className={cn(
        "relative flex items-center overflow-hidden justify-center size-6 shrink-0 bg-bg-elevated text-text-muted rounded-full text-xs data-[state=completed]:bg-primary data-[state=completed]:text-text-inverse data-[state=active]:bg-primary data-[state=active]:text-text-inverse",
        className
      )}
    >
...
function StepperSeparator({ className }: React.ComponentProps<"div">) {
  ...
  return (
    <div
      ...
      data-state={state}
      className={cn(
        "m-0.5 rounded-full bg-border group-data-[orientation=vertical]/stepper-nav:h-12 group-data-[orientation=vertical]/stepper-nav:w-0.5 group-data-[orientation=horizontal]/stepper-nav:h-0.5 group-data-[orientation=horizontal]/stepper-nav:flex-1 data-[state=completed]:bg-primary",
        className
      )}
    />
  )
...
function StepperTitle({ children, className }: React.ComponentProps<"h3">) {
  ...
  return (
    <h3
      ...
      data-state={state}
      className={cn(
        "text-xs font-semibold leading-none group-data-[state=inactive]/step:text-text-muted group-data-[state=active]/step:text-text-primary group-data-[state=completed]/step:text-primary",
        className
      )}
    >
      {children}
